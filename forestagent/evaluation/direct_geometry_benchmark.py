"""Small-sample direct geometry baseline benchmark for q1/q2."""
...
class DirectGeometryFailureReasonStat(BaseModel):
...
class DirectGeometryBenchmarkResult(BaseModel):
    """Structured output of the direct geometry q1/q2 benchmark."""
    ...
    failure_reason_stats: list[DirectGeometryFailureReasonStat]


def benchmark_direct_geometry_baseline(
    ...
    """Run a q1/q2 benchmark with the direct geometry backend."""
    ...
    if not selected_sample_ids:
        raise ValueError("No sample_ids were selected for the direct geometry benchmark.")

    backend = backend or DirectGeometryBackend(config_path=config_path)
    ...
    return DirectGeometryBenchmarkResult(
        sample_ids=selected_sample_ids,
        modality=modality,
...
def _build_failure_reason_stats(
    records: list[EvaluationRecord],
) -> list[DirectGeometryFailureReasonStat]:
